{* Template Name:分类资讯模板 * Template Type:catainfo *}<?php echo'<meta charset="UTF-8"><div style="text-align:center;padding:60px 0;font-size:16px;"><h2 style="font-size:60px;margin-bottom:32px;color:f00;">欢迎您的访问！</h2><h3>但是，这里并没有你想找的东西，喜欢这个的主题可以联系我们！</h3><h2 style="font-size:50px;margin-bottom:32px;color:f00;">QQ：229693666</h2></div>';die();?>{template:header}
...
<main class="main-content container clearfix">
	...
	{if $type=='category'}<div class="filter-sift">
		...
		{if $category->Level == 0}{$subcate = $zbp->GetCategoryList('*',array(array('=','cate_ParentID',$category->ID)),array('cate_Order'=>'ASC'))}
		{elseif $category->Level >= 1}{$subcate = $zbp->GetCategoryList('*',array(array('=','cate_ParentID',$category->ParentID)),array('cate_Order'=>'ASC'))}
		{/if}{if isset($subcate) && $subcate}
		<ul class="filter-tag">
			<span class="filter-l"><i class="icon font-layers"></i>子类</span>
			<li class="clearfix">
				{foreach $subcate as $cate}
				<a id="cate-{$cate->ID}" href="{$cate->Url}" class="{if $cate->Url == $category->Url}current{/if}">{$cate->Name}</a>
				{/foreach}
			</li>
		</ul>{/if}
	...
	<div class="auto-wrap">
		<div class="catalist-main auto-side">
			...
			{if $article.IsTop}
				{template:post-infotop}
            {else}
				{if $zbp->Config('downlee')->sycmsadoff=="1"}{if $i==4}{template:post-infoads}{/if}{php}$i++;{/php}{/if}
				{template:post-infotab}
            {/if}{/foreach}
			</div>
