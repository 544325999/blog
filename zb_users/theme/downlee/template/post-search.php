...
<article class="info-list auto-list{if $zbp->Config('downlee')->gdtxoff=='1'} wow fadeInDown" data-wow-delay="0.25s{/if}">
	...
	<div class="sl-main">
		...
		<div class="sl-info">{php}$description = preg_replace('/[\r\n]+/', '', trim(SubStrUTF8(TransferHTML($article->Content,'[nohtml]'),168)).'...');echo str_replace($search,'<span style="color:red">'.$search.'</span>',$description){/php}</div>
	</div>
