...
<article class="item-body auto-list{if $zbp->Config('downlee')->gdtxoff=='1'} wow fadeInDown{/if}" id="post-{$i}">
	...
	<div class="item-wrapper">
		...
		<header class="item-header">
			<h2 class="item-title"><a href="{$article.Url}" title="{$article.Title}" rel="bookmark"{if $zbp->Config('downlee')->blankoff=='1'} target="_blank"{/if}><span class="badge arc_v5">置顶</span>{$article.Title}</a></h2>
		</header>
