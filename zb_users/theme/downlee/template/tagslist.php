{* Template Name:标签聚合单页 * Template Type:tagslist *}<?php echo'<meta charset="UTF-8"><div style="text-align:center;padding:60px 0;font-size:16px;"><h2 style="font-size:60px;margin-bottom:32px;color:f00;">欢迎您的访问！</h2><h3>但是，这里并没有你想找的东西，喜欢这个的主题可以联系我们！</h3><h2 style="font-size:50px;margin-bottom:32px;color:f00;">QQ：229693666</h2></div>';die();?>{template:header}
			...
			<div class="article-content">
				<div class="single-entry">
					...
					<div id="tagslist" class="linkcat">
						<h2>{$article.Title}</h2>
						<ul class="tag_could">{$tags = $zbp->GetTagList('*',null,array('tag_Count'=>'DESC'))}{foreach $tags as $tag}
							<li><a href="{$tag.Url}" title="关于[{$tag.Name}]的文章共有{$tag.Count}篇"><span>{$tag.Name}</span><em>共{$tag.Count}篇文章</em></a></li>{/foreach}
						</ul>
					</div>
				</div>
