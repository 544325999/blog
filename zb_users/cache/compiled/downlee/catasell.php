<?php  /* Template Name:分类商品模板 * Template Type:catasell */  ?><?php  include $this->GetTemplate('header');  ?>
...
<main class="main-content container clearfix">
	...
	<?php if ($type=='category') { ?><div class="filter-sift">
		...
		<?php if ($category->Level == 0) { ?>
			<?php  $subcate = $zbp->GetCategoryList('*',array(array('=','cate_ParentID',$category->ID)),array('cate_Order'=>'ASC'));  ?>
		<?php }elseif($category->Level == 1) {  ?>
			<?php  $subcate = $zbp->GetCategoryList('*',array(array('=','cate_ParentID',$category->ParentID)),array('cate_Order'=>'ASC'));  ?>
		<?php }elseif($category->Level == 2) {  ?>
			<?php  $subcate = $zbp->GetCategoryList('*',array(array('=','cate_ParentID',$category->Parent->ParentID)),array('cate_Order'=>'ASC'));  ?>
		<?php } ?>
		<?php if (isset($subcate) && $subcate) { ?>
		<ul class="filter-tag">
			<span class="filter-l"><i class="icon font-layers"></i>子类</span>
			<li class="clearfix">
				<?php  foreach ( $subcate as $cate) { ?>
				<a id="cate-<?php  echo $cate->ID;  ?>" href="<?php  echo $cate->Url;  ?>" class="<?php if ($cate->Url == $category->Url || $category->Level == 2 && $category->Parent->Url == $cate->Url) { ?>current<?php } ?>"><?php  echo $cate->Name;  ?></a>
				<?php }   ?>
			</li>
		</ul><?php } ?>
	...
	<div class="category-main auto-side">
		...
		<div class="catesell-item auto-main">
			...
			<?php if ($article->IsTop) { ?>
				<?php  include $this->GetTemplate('post-selltop');  ?>
            <?php }else{  ?>
				<?php if ($zbp->Config('downlee')->sycmsadoff=="1") { ?><?php if ($i==4) { ?><?php  include $this->GetTemplate('post-sellads');  ?><?php } ?><?php $i++; ?><?php } ?>				
				<?php  include $this->GetTemplate('post-sellist');  ?>
            <?php } ?><?php }   ?>
		</div>
